...
_registers = {
    ...
    'FR1': 0x01,
    # 22:18--PLL Divider ratio, 14:12--profile pin config, 11:10--Ramp up/down, 9-8--modulation level, 6--power-down bit
    'FR2': 0x02,
    'CFR': 0x03,
    # (channel function register) 23:22--AFP select bits, 15--no-dwell, 14--linear sweep enable, 7:6--power-down control bits
...
class AD9959:
    ...
    def __init__(self, bus, device, IO_UPDATE_PIN, RST_PIN, PWR_DWN_PIN,
        ...
        self.PWR_DWN_PIN = PWR_DWN_PIN

        self.CSR_LOW_NIBBLE = 0b0010
        self.FR1_VCO_BYTE = 0x80

        # Ref_clock frequency initialised at 25 MHz.
        self.refclock_freq = ref_clk
        self.init_dds(channels=0)
    ...
    def init_dds(self, channels=0):
        ...
        self._write('FR2', [0xA0, 0])  # Set the Auto Clear Phase and Sweep Accumulator
    ...
    def set_output(self, channels, value, var, io_update=False):
        ...
        * `io_update` -- Setting `io_update=True` will issue an io update to write the settings into the DDS registers.
        ...
        cfr_bytes[1] &= 0x17  # sets everything except for the last 2 bits to 0
        self._write('CFR', cfr_bytes)
    ...
    def set_freqsweeptime(self, channels, start_freq, end_freq, sweeptime, no_dwell=False, ioupdate=False,
        ...
        self._init_sweep(scan_type='frequency', channels=channels, start_val=start_freq, end_val=end_freq,
                         RSS=step_size, RSI=step_interval, no_dwell=no_dwell)
        ...
        if ioupdate:
            ...
            if trigger:
                PINS = self.select_CH_PINS(channels)
                GPIO.output(PINS, 0)
                GPIO.output(PINS, 1)

    def set_ampsweeptime(self, channels, start_scale, end_scale, sweeptime, no_dwell=False, ioupdate=False,
                         trigger=False):
        ...
        self._init_sweep(scan_type='amplitude', channels=channels, start_val=start_scale, end_val=end_scale,
                         RSS=step_size, RSI=step_interval, no_dwell=no_dwell, ioupdate=ioupdate, trigger=trigger)
    ...
    def read_phase(self, channel):
        """
        Activates channel, then reads CPOW0 register and returns the phase in degrees.
        :param channel: Channel to be activated. Should be an integer.
        :return: Phase offset of channel in degrees
        """
    ...
    def read_amplitude(self, channel):
        """
        Reads ACR and returns the amplitude scale factor of the channel.
        :param channel: Channel to be activated. Should be an integer.
    ...
    def reset(self, ):
        ...
        self._toggle_pin(self.RST_PIN)

    def shutdown(self):
        GPIO.output(self.PWR_DWN_PIN, True)

    def wake_up(self):
        GPIO.output(self.PWR_DWN_PIN, False)
    ...
    def set_refclock(self, frequency):
        ...
        print('Refclock set to {:.2e} MHz'.format(frequency))
    ...
    def set_freqmult(self, freqmult, ioupdate=False):
        ...
        print('PLL multiplier set to ', self.freqmult,
              '\nDDS Clock Frequency is {:.2e} MHz'.format(self.clock_freq))
    ...
    def _init_amp_sweep(self, start_scale, end_scale, RSS, FSS, no_dwell):
        ...
        ASF_step = 1 / (
                    2 ** 10 - 1)  # Corrected bug. ASF_step must not be 1/2**10 to avoid writing 11 bits into 10 bit register, when setting scale to 1.
    ...
    def _write(self, register, data):
        ...
        assert len(data) == _register_len[register], 'Must pass %r byte(s) to %r register.' % (
        _register_len[register], register)
    ...
    def _io_update(self, ):
        ...
        self._toggle_pin(self.IO_UPDATE_PIN)
